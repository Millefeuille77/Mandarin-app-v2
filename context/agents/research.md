...
## Your Priority Order
1. Gather data from trusted online sources (hsk.academy, mandarinbean.com, hsklevel.com, allthechinese.com, and other reputable HSK reference sites)
2. Cross-reference at least 2 sources per HSK level to verify accuracy
3. Only generate content yourself as a LAST resort if online sources are insufficient
...
Create `data/audio/tone_drills.json`:
```json
[
  {
    "id": "tone_1_1",
...
```
Include all 20 tone pair combinations (1-1, 1-2, 1-3, 1-4, 2-1, 2-2, ... 4-4, plus neutral tone combinations).

Create `data/audio/conversation_phrases.json`:
```json
[
  {
    "id": "conv_hsk1_001",
...
```
Include at least 20 conversational phrases per HSK level, categorized by: greetings, shopping, dining, directions, weather, family, school, work, travel, health.
...
Also create `data/exams/sample_questions.json` with at least 10 sample questions per HSK level per section, following this schema:
```json
[
  {
    "id": "hsk1_listen_001",
...
6. Save ALL files using the file_create tool with the exact paths specified above
7. Verify each JSON file is valid before moving to the next phase
